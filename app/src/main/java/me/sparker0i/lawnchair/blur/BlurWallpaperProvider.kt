package me.sparker0i.lawnchair.blur
...
class BlurWallpaperProvider(context: Context) {
    ...
    private val mNotifyRunnable = Runnable {
        if (me.sparker0i.lawnchair.Utilities.getPrefs(context).centerWallpaper)
    ...
    init {
        isEnabled = mWallpaperManager.wallpaperInfo == null && me.sparker0i.lawnchair.Utilities.getPrefs(context).enableBlur
    ...
    private fun updateBlurRadius() {
        blurRadius = me.sparker0i.lawnchair.Utilities.getPrefs(context).blurRadius.toInt() / DOWNSAMPLE_FACTOR
        blurRadius = Math.max(1, Math.min(blurRadius, 25))
    }

    private val enabledFlag: Int
        get() = me.sparker0i.lawnchair.Utilities.getPrefs(context).blurMode

    private fun updateWallpaper() {
        val launcher = me.sparker0i.lawnchair.LauncherAppState.getInstance().launcher
        val enabled = mWallpaperManager.wallpaperInfo == null && me.sparker0i.lawnchair.Utilities.getPrefs(context).enableBlur
        ...
        if (me.sparker0i.lawnchair.Utilities.getPrefs(context).enableVibrancy) {
    ...
    val tintColor: Int
        get() = me.sparker0i.lawnchair.Utilities.resolveAttributeData(context, R.attr.blurTintColor)

    fun updateAsync() {
        me.sparker0i.lawnchair.Utilities.THREAD_POOL_EXECUTOR.execute(mUpdateRunnable)
    }
    ...
    companion object {
        ...
        fun applyBlurBackground(activity: Activity) {
            ...
            var color = me.sparker0i.lawnchair.Utilities.resolveAttributeData(activity, R.attr.blurTintColor)
        ...
        val instance: BlurWallpaperProvider
            get() = me.sparker0i.lawnchair.LauncherAppState.getInstance().launcher.blurWallpaperProvider
    }
}
