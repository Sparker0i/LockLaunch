...
class LauncherClientImpl(private val launcher: me.sparker0i.lawnchair.Launcher, targetPackage: String, overlayEnabled: Boolean) : ILauncherClient {
    ...
    constructor(launcher: me.sparker0i.lawnchair.Launcher, overlayEnabled: Boolean) : this(launcher, "com.google.android.googlequicksearchbox", overlayEnabled)

    private fun applyWindowToken() {
        if (!isConnected || !me.sparker0i.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            return
        }
    ...
    override fun onAttachedToWindow() {
        if (!destroyed && me.sparker0i.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            setWindowAttrs(launcher.window.attributes)
        }
    }

    override fun onDestroy() {
        if (me.sparker0i.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
    ...
    fun reconnect() {
        if (destroyed || state != 0 || !me.sparker0i.lawnchair.Utilities.getPrefs(launcher).showGoogleNowTab) {
            return
        }
